...
class Proxy(multiprocessing.Process):
    ...
    def _process_rlist(self, r):
	    ...
	    print("-- client", data)
	    parser = HttpParser()
	    parser.process_line(data)
            cache = Cache()
            value = cache.get_if_exists(parser.orig_url)
            if value:
                print(value)
                self.client.send("""HTTP/1.1 200 OK
Server: nginx
Date: Sat, 08 Apr 2017 07:09:07 GMT
Content-Type: text/html
Content-Length: 2
Last-Modified: Sun, 24 Apr 2016 21:21:50 GMT
